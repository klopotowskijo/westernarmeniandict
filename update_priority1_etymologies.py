import json
import csv

# File paths
DICT_PATH = "western_armenian_merged_final_complete.json"
CSV_PATH = "priority1_etymologies_partial.csv"
OUTPUT_PATH = "western_armenian_merged_priority1_complete.json"

# Load dictionary
with open(DICT_PATH, 'r', encoding='utf-8') as f:
    data = json.load(f)

# Load CSV
csv_rows = {}
with open(CSV_PATH, 'r', encoding='utf-8') as f:
    import json
    import csv

    DICT_PATH = "western_armenian_merged_final_complete.json"
    CSV_PATH = "priority1_etymologies_partial.csv"
    OUTPUT_PATH = "western_armenian_merged_priority1_complete.json"

    with open(DICT_PATH, 'r', encoding='utf-8') as f:
        data = json.load(f)

    csv_rows = {}
    with open(CSV_PATH, 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        for row in reader:
            csv_rows[row['title'].strip()] = row

    matched = 0
    updated = 0
    not_found = []

    PLACEHOLDER_STRINGS = [
        'TBD', 'Unknown', 'needs research', 'Proper name', 'etymology uncertain', 'Etymology needs research', 'No usable data', 'N/A', '', None
    ]
    def is_placeholder(etym):
        if not etym:
            return True
        if isinstance(etym, dict):
            text = etym.get('text', '')
        else:
            text = str(etym)
        text = text.strip().lower()
        for ph in PLACEHOLDER_STRINGS:
            if ph and ph.lower() in text:
                return True
        return False

    for entry in data:
        title = entry.get('title', '').strip()
        if title in csv_rows:
            matched += 1
            row = csv_rows[title]
            new_etym = row['new_etymology'].strip()
            relation = row['relation'].strip()
            current_etym = entry.get('etymology', {})
            if is_placeholder(current_etym):
                entry['etymology'] = {
                    'text': new_etym,
                    'relation': relation,
                    'source': 'priority1_batch_fix'
                }
                updated += 1
        else:
            not_found.append(title)

    with open(OUTPUT_PATH, 'w', encoding='utf-8') as f:
        json.dump(data, f, ensure_ascii=False, indent=2)

    print(f"Total matched: {matched}")
    print(f"Total updated: {updated}")
    print(f"Not found in dictionary: {len(not_found)}")
    if not_found:
        print("Sample not found:", not_found[:10])

    # Debug: print sample titles from both sources
    print("Sample CSV titles:", list(csv_rows.keys())[:10])
    print("Sample JSON titles:", [entry.get('title', '') for entry in data[:10]])

    # 6. Save output
    with open(JSON_OUT_PATH, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)

    # 7. Generate report
    print(f"Total updated: {updated}")
    if not_found:
        print(f"Words not found in JSON: {len(not_found)}")
        print(not_found[:10])
    else:
        print("All words from CSV matched in JSON.")
    print("Sample of changes:")
    for c in changes[:5]:
        print(f"{c['title']}:\n  Old: {c['old']}\n  New: {c['new']}")

